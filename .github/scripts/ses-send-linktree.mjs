...
// ── File paths ──────────────────────────────────────────────────────
const INPUT_PATH  = join(__dirname, 'linktree-verified.json');
const SENT_PATH   = join(__dirname, 'linktree-sent.json');
const LOGS_PATH   = join(__dirname, 'linktree-logs.json');
...
console.log(`📧 Linktree A/B Campaign — Sending ${batch.length} emails via SES\n`);
