...
export BATCH_SIZE="${BATCH_SIZE:-1200}"
export CONCURRENCY="${CONCURRENCY:-40}"
export RE_ENRICH=1
export TURBO=1
export CONTINUOUS=1
...
source <(grep -E '^(SUPABASE_URL|NEXT_PUBLIC_SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SUPABASE_KEY|GEMINI_API_KEY|GEMINI_API_KEY_[0-9]|GEMINI_MODEL|GROQ_API_KEY|GROQ_API_KEY_[0-9]|OPENAI_API_KEY|OPENAI_API_KEY_[0-9]|OPENAI_MODEL|COHERE_API_KEY|COHERE_API_KEY_[0-9]|ANTHROPIC_API_KEY)=' .env.local 2>/dev/null | sed 's/"//g')
set +a
  ...
  (
    ...
    while true; do
      ...
      export WORKER_ID="$w" WORKERS="$WORKERS" BATCH_SIZE="$BATCH_SIZE" \
        ...
        RE_ENRICH=1 TURBO=1 OPENAI_FAST_MODEL="$OPENAI_FAST_MODEL"
