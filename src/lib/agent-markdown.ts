  ...
  switch (page as MarkdownPageKey) {
    case 'home': {
      ...
      return [
        ...
        '',
        `CVin.Bio turns your CV into a professional personal website and runs a curated tech job board. Upload a PDF or Word resume and AI extracts your work history, education, and skills to build a shareable profile page at cvin.bio/yourname — free, in seconds. Learn more on our [About CVin.Bio](${siteUrl}/about) page.`,
        '',
        `The [Curated Tech Job Board](${siteUrl}/jobs) lists ${stats.jobCountDisplay} live tech roles at companies like OpenAI, Anthropic, Stripe, and Cloudflare. Explore all active employers in our [Hiring Companies Directory](${siteUrl}/companies), browse residencies in our [Tech Fellowships Board](${siteUrl}/fellowships), or read community reports on our [Discover Hub](${siteUrl}/discover). Every public profile is matched against open roles by skills and location.`,
        '',
        `## API Endpoints & Authentication`,
        '',
        `- Base API URL: ${siteUrl}/api`,
        ...
        `## REST API Endpoints`,
        '',
        `- GET ${siteUrl}/api/jobs?page=1&limit=20&q=&type=&loc=remote — Paginated feed of curated tech jobs. Supports cursor pagination (next_cursor).`,
        `- GET ${siteUrl}/api/jobs/{id} — Full detail for one public job listing.`,
        `- GET ${siteUrl}/api/news?limit=10 — Aggregated tech news from tier-1 sources.`,
        `- GET ${siteUrl}/api/profile/{username} — Public candidate profile details.`,
        '',
        `## Machine Specs & Open Standards`,
        '',
        `- [OpenAPI 3.1 Specification](${siteUrl}/openapi.json) — conforming to the [OpenAPI Standard](https://openapis.org)`,
        `- [Model Context Protocol (MCP) Specification](${siteUrl}/.well-known/mcp.json) — conforming to the [MCP Open Spec](https://modelcontextprotocol.io)`,
        `- [OAuth Authorization Server Metadata](${siteUrl}/.well-known/oauth-authorization-server) — published per [IETF RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) and [WorkOS Agentic Auth Spec](https://workos.com/auth-md)`,
        `- [Agent Skills Index](${siteUrl}/.well-known/agent-skills/index.json) — schema per [Agent Skills Discovery v0.2.0](https://schemas.agentskills.io)`,
        `- [Web Accessibility Standards](https://www.w3.org/WAI/standards-guidelines/) — semantics and keyboard accessibility`,
        `- Agent Instructions: ${siteUrl}/agent.txt or ${siteUrl}/agent-instructions.txt`,
        `- llms.txt Directory: ${siteUrl}/llms.txt and ${siteUrl}/llms-full.txt`,
        '',
        `## Model Context Protocol (MCP) Server`,
        '',
        `- MCP Manifest: ${siteUrl}/.well-known/mcp.json and ${siteUrl}/.well-known/mcp`,
        `- Streamable HTTP Endpoint: POST ${siteUrl}/mcp (JSON-RPC 2.0)`,
        `- Available MCP Tools: search_jobs, get_job, platform_stats`,
        '',
        `## Core Pages & Directories`,
        '',
        `- [CVin.Bio Homepage](${siteUrl})`,
        `- [Curated Tech Job Board](${siteUrl}/jobs)`,
        `- [Hiring Companies Directory](${siteUrl}/companies)`,
        `- [Tech Fellowships Board](${siteUrl}/fellowships)`,
        `- [Discover Directory & Insights](${siteUrl}/discover)`,
        `- [Developer Documentation & API](${siteUrl}/docs)`,
        `- [About CVin.Bio](${siteUrl}/about)`,
