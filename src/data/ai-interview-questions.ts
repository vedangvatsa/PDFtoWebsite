...
model = get_peft_model(base_model, lora_config)`,
 ...
 keyPoints: [
 'SFT: Direct training on selected (prompt, completion) pairs; teaches format and task behavior.',
...
function getIoU(boxA, boxB) {
 ...
 'Purpose: Prevent overfitting, handle real-world variation, reduce data collection needs.',
...
async function multimodalRAG(query) {
 ...
 'Domain eval: selected set of domain-specific images with expert-verified descriptions.',
...
Revision: "I cannot generate exploit code, but I can detail network security concepts."`
 ...
 idealAnswer: {
 coreIdea: 'Over-refusal is when the model rejects safe requests that it mistakes as harmful. Measure it with a selected safe-but-sensitive test set, then reduce it by refining safety thresholds and prompt instructions.',
 keyPoints: [
 'Over-refusal test set: selected examples of safe requests that look risky (medical questions, violence in fiction, security research).',
...
async function routeOutput(output, context) {
 ...
 'Sequence length: LayerNorm normalizes each token on its own, so variable sequence lengths are fine.',
