  ...
  return (
    <svg viewBox={`0 0 ${totalW} ${top + chartH + 50}`} fill="none" className="w-full h-auto" aria-hidden="true">
      {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => {
        ...
        return (<g key={i}><line x1="45" y1={y} x2={totalW - 10} y2={y} className="stroke-zinc-100" strokeWidth="0.5" /><text x="40" y={y + 4} textAnchor="end" className="fill-zinc-400 text-[9px]" fontFamily="inherit">${Math.round(pct * max)}B</text></g>);
      })}
      {data.map((d, i) => {
        const x = 50 + i * (barW + gap);
        const bH = (d.value / max) * chartH;
        const y = top + chartH - bH;
        return (<g key={i}><rect x={x} y={y} width={barW} height={bH} rx={4} className={i >= 4 ? 'fill-zinc-300' : 'fill-zinc-900'} /><text x={x + barW / 2} y={y - 8} textAnchor="middle" className="fill-zinc-700 text-[10px] font-semibold" fontFamily="inherit">{d.label}</text><text x={x + barW / 2} y={top + chartH + 28} textAnchor="middle" className="fill-zinc-500 text-[10px]" fontFamily="inherit">{d.year}</text></g>);
      })}
      <text x={totalW - 12} y={top + chartH + 46} textAnchor="end" className="fill-zinc-400 text-[7px]" fontFamily="inherit">*Projected. Sources: Research Nester, Mordor Intelligence</text>
    </svg>
  );
}
...
function MarketSizeDiagram() {
  return (
    <svg viewBox="0 0 400 280" fill="none" className="w-full h-auto" aria-hidden="true">
      {/* TAM outer */}
      <circle cx="200" cy="150" r="125" className="fill-zinc-100" />
      <text x="200" y="50" textAnchor="middle" className="fill-zinc-500 text-[11px] font-bold" fontFamily="inherit">TAM</text>
      <text x="200" y="66" textAnchor="middle" className="fill-zinc-400 text-[9px]" fontFamily="inherit">$640B global recruitment</text>
      {/* SAM middle */}
      <circle cx="200" cy="165" r="80" className="fill-zinc-300" />
      <text x="200" y="112" textAnchor="middle" className="fill-zinc-600 text-[11px] font-bold" fontFamily="inherit">SAM</text>
      <text x="200" y="128" textAnchor="middle" className="fill-zinc-500 text-[9px]" fontFamily="inherit">$17B online recruitment tech</text>
      {/* SOM inner */}
      <circle cx="200" cy="185" r="40" className="fill-zinc-900" />
      <text x="200" y="180" textAnchor="middle" className="fill-white text-[11px] font-bold" fontFamily="inherit">SOM</text>
      <text x="200" y="196" textAnchor="middle" className="fill-white/70 text-[9px]" fontFamily="inherit">AI-native niche</text>
      {/* Sources */}
      <text x="200" y="276" textAnchor="middle" className="fill-zinc-400 text-[8px]" fontFamily="inherit">Staffing Industry Analysts, Tracxn, EIN Presswire</text>
    </svg>
  );
}
...
function CostComparison() {
  return (
    <svg viewBox="0 0 420 140" fill="none" className="w-full h-auto" aria-hidden="true">
      {/* Recruiter agency */}
      <rect x="15" y="15" width="180" height="110" rx="12" className="fill-white stroke-zinc-200" strokeWidth="1" />
      <text x="105" y="42" textAnchor="middle" className="fill-zinc-400 text-[10px] uppercase tracking-widest" fontFamily="inherit">Recruiter Agency</text>
      <text x="105" y="78" textAnchor="middle" className="fill-zinc-700 text-[24px] font-bold" fontFamily="inherit">$40-60K</text>
      <text x="105" y="100" textAnchor="middle" className="fill-zinc-400 text-[10px]" fontFamily="inherit">20-30% of $200K salary</text>
      <text x="105" y="115" textAnchor="middle" className="fill-zinc-400 text-[9px]" fontFamily="inherit">89+ days avg. time-to-fill</text>
      {/* Arrow */}
      <line x1="202" y1="70" x2="218" y2="70" className="stroke-zinc-300" strokeWidth="1.5" />
      <polygon points="218,66 226,70 218,74" className="fill-zinc-300" />
      {/* CVin.Bio */}
      <rect x="230" y="15" width="180" height="110" rx="12" className="fill-zinc-900" />
      <text x="320" y="42" textAnchor="middle" className="fill-zinc-400 text-[10px] uppercase tracking-widest" fontFamily="inherit">CVin.Bio</text>
      <text x="320" y="78" textAnchor="middle" className="fill-white text-[24px] font-bold" fontFamily="inherit">Fraction</text>
      <text x="320" y="100" textAnchor="middle" className="fill-white/70 text-[10px]" fontFamily="inherit">Structured discovery</text>
      <text x="320" y="115" textAnchor="middle" className="fill-white/70 text-[9px]" fontFamily="inherit">Instant skill-matched results</text>
    </svg>
  );
}
...
function CompetitiveLandscape() {
  const incumbents = [
    { name: 'LinkedIn', x: 70, y: 55, r: 34, rev: '$17.1B', color: 'fill-zinc-200' },
    { name: 'Indeed', x: 170, y: 70, r: 28, rev: '', color: 'fill-zinc-200' },
    { name: 'ZipRecruiter', x: 285, y: 58, r: 20, rev: '$449M', color: 'fill-zinc-200' },
    { name: 'Wellfound', x: 60, y: 145, r: 14, rev: '', color: 'fill-zinc-200' },
    { name: 'Otta', x: 140, y: 155, r: 12, rev: '', color: 'fill-zinc-200' },
    { name: 'Hired', x: 210, y: 150, r: 12, rev: '', color: 'fill-zinc-200' },
  ];
  const aiPlayers = [
    { name: 'Juicebox AI', x: 340, y: 90, r: 22, rev: '$850M val', color: 'fill-amber-200' },
    { name: 'Eightfold AI', x: 390, y: 52, r: 20, rev: '', color: 'fill-amber-100' },
    { name: 'SeekOut', x: 455, y: 80, r: 16, rev: '', color: 'fill-amber-100' },
    { name: 'Moonhub', x: 300, y: 145, r: 14, rev: '', color: 'fill-amber-100' },
    { name: 'hireEZ', x: 385, y: 140, r: 14, rev: '', color: 'fill-amber-100' },
  ];
  return (
    <svg viewBox="0 0 580 310" fill="none" className="w-full h-auto" aria-hidden="true">
      {/* Axis labels */}
      <text x="290" y="300" textAnchor="middle" className="fill-zinc-400 text-[10px]" fontFamily="inherit">General purpose ← → Domain-specific / AI-native</text>
      <text x="14" y="155" textAnchor="middle" className="fill-zinc-400 text-[10px]" fontFamily="inherit" transform="rotate(-90 14 155)">Scale</text>
      {/* Grid */}
      <line x1="30" y1="270" x2="555" y2="270" className="stroke-zinc-200" strokeWidth="0.5" />
      <line x1="30" y1="25" x2="30" y2="270" className="stroke-zinc-200" strokeWidth="0.5" />
      {/* Vertical divider */}
      <line x1="255" y1="25" x2="255" y2="270" className="stroke-zinc-200" strokeWidth="0.5" strokeDasharray="4 3" />
      <text x="140" y="265" textAnchor="middle" className="fill-zinc-300 text-[9px] font-semibold" fontFamily="inherit">Job boards / marketplaces</text>
      <text x="410" y="265" textAnchor="middle" className="fill-amber-400/70 text-[9px] font-semibold" fontFamily="inherit">AI sourcing tools</text>
      {/* Incumbents */}
      {incumbents.map((p, i) => (
        <g key={`inc-${i}`}>
          <circle cx={p.x} cy={p.y} r={p.r} className={p.color} />
          <text x={p.x} y={p.y + 3} textAnchor="middle" className="fill-zinc-600 text-[10px] font-bold" fontFamily="inherit">{p.name}</text>
          {p.rev && <text x={p.x} y={p.y + 15} textAnchor="middle" className="fill-zinc-400 text-[8px]" fontFamily="inherit">{p.rev}</text>}
        </g>
      ))}
      {/* AI players */}
      {aiPlayers.map((p, i) => (
        <g key={`ai-${i}`}>
          <circle cx={p.x} cy={p.y} r={p.r} className={p.color} />
          <text x={p.x} y={p.y + 3} textAnchor="middle" className="fill-zinc-700 text-[9px] font-bold" fontFamily="inherit">{p.name}</text>
          {p.rev && <text x={p.x} y={p.y + 15} textAnchor="middle" className="fill-zinc-400 text-[7px]" fontFamily="inherit">{p.rev}</text>}
        </g>
      ))}
      {/* CVin.Bio - positioned in the unique gap: structured + agent-queryable */}
      <rect x="415" y="170" width="135" height="90" rx="10" className="stroke-zinc-400" strokeWidth="1.5" strokeDasharray="5 3" fill="none" />
      <text x="482" y="166" textAnchor="middle" className="fill-zinc-500 text-[9px] font-semibold" fontFamily="inherit">Agent-queryable infrastructure</text>
      <circle cx="482" cy="215" r="22" className="fill-zinc-900" />
      <text x="482" y="212" textAnchor="middle" className="fill-white text-[9px] font-bold" fontFamily="inherit">CVin.Bio</text>
      <text x="482" y="224" textAnchor="middle" className="fill-white/70 text-[7px]" fontFamily="inherit">MCP + structured data</text>
    </svg>
  );
}
...
function PlatformStack({ statsLabel }: { statsLabel: string }) {
  return (
    <svg viewBox="0 0 520 200" fill="none" className="w-full h-auto" aria-hidden="true">
      <rect x="30" y="145" width="460" height="48" rx="8" className="fill-zinc-200" />
      <text x="260" y="166" textAnchor="middle" className="fill-zinc-600 text-[11px] font-bold" fontFamily="inherit">Multi-source job aggregation</text>
      <text x="260" y="182" textAnchor="middle" className="fill-zinc-400 text-[9px]" fontFamily="inherit">{statsLabel}</text>
      <rect x="60" y="90" width="400" height="48" rx="8" className="fill-zinc-400" />
      <text x="260" y="111" textAnchor="middle" className="fill-white text-[11px] font-bold" fontFamily="inherit">Structured candidate profiles</text>
      <text x="260" y="127" textAnchor="middle" className="fill-white/70 text-[9px]" fontFamily="inherit">Schema.org, JSON-LD, skill extraction</text>
      <rect x="90" y="35" width="340" height="48" rx="8" className="fill-zinc-900" />
      <text x="260" y="56" textAnchor="middle" className="fill-white text-[11px] font-bold" fontFamily="inherit">AI-native reverse discovery</text>
      <text x="260" y="72" textAnchor="middle" className="fill-white/60 text-[9px]" fontFamily="inherit">MCP server, llms.txt, 100+ AI crawlers</text>
      <line x1="260" y1="83" x2="260" y2="90" className="stroke-zinc-300" strokeWidth="1" strokeDasharray="4 3" />
      <line x1="260" y1="138" x2="260" y2="145" className="stroke-zinc-300" strokeWidth="1" strokeDasharray="4 3" />
    </svg>
  );
}
      ...
      <main id="main-content" className={PAGE_CONTAINER}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center mb-24">
          <div>
            ...
            <h1 className="text-4xl sm:text-[3.4rem] font-serif font-bold tracking-tight text-zinc-900 mb-8 leading-[1.12]">
          ...
          <div className="hidden lg:block">
            <div className="bg-white border border-zinc-200 rounded-2xl p-6">
              <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-5 text-center">Autonomous AI agent market growth</p>
              <MarketGrowthChart />
            </div>
          </div>
        </div>
        ...
        {/* ─── BIG NUMBERS ─── */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-zinc-200 rounded-2xl overflow-hidden mb-28">
          {[
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">The labor market is restructuring around AI agents and nobody is serving the new demand</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div>
              <div className="bg-white border border-zinc-200 rounded-2xl p-8 mb-4">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-6">Roles most in demand, AI-era companies</p>
                <HBar data={[
              ...
              <div className="grid grid-cols-2 gap-px bg-zinc-200 rounded-2xl overflow-hidden">
                {[
                  ...
                  <div key={i} className="bg-[#fafafa] p-5 text-center">
                    <div className="text-2xl font-serif font-bold text-zinc-900 tracking-tight">{d.value}</div>
                    <div className="text-[10px] text-zinc-400 mt-1.5 whitespace-pre-line">{d.label}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Hiring talent costs too much, takes too long, and produces poor signal</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              <p className="text-[15px] text-zinc-500 leading-[1.85] mb-5">
            ...
            <div>
              <div className="bg-white border border-zinc-200 rounded-2xl p-8">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-6 text-center">Cost of one senior engineering hire via recruiter</p>
                <CostComparison />
              </div>
            </div>
          </div>
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Large market with no dominant vertical player</h2>
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-2 flex items-center">
              <div className="bg-white border border-zinc-200 rounded-2xl p-6 w-full">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-4 text-center">Market layers</p>
                <MarketSizeDiagram />
              </div>
            </div>
          </div>
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Four integrated layers, each feeding the others</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center mb-10">
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl p-6">
              <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-4 text-center">Platform architecture</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[
              ...
              <div key={card.n} className="p-6 rounded-xl border border-zinc-200 bg-white">
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex items-center justify-center w-7 h-7 rounded-full bg-zinc-900 text-white font-bold text-xs">{card.n}</div>
                  <h3 className="text-sm font-bold text-zinc-900">{card.title}</h3>
                </div>
                <ul className="space-y-2">
                  {card.points.map((pt, i) => (
                    <li key={i} className="text-[13px] text-zinc-500 leading-[1.7] flex items-start gap-2">
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Well-funded competitors automate recruiters. We replace the need for them.</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center mb-12">
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl p-8">
              <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-6 text-center">Where we sit</p>
          ...
          <div className="mb-10">
            <p className="text-[10px] text-zinc-400 uppercase tracking-[0.2em] font-semibold mb-4">Tier 1 · Horizontal job platforms</p>
            <div className="space-y-2">
              {[
                ...
                <p key={i} className="text-[13px] text-zinc-500 leading-[1.7] flex items-start gap-2">
                  <span className="w-1 h-1 rounded-full bg-zinc-400 mt-[7px] shrink-0" />
                  <span><span className="font-semibold text-zinc-700">{item.bold}</span> {item.rest}</span>
                </p>
              ))}
            </div>
          </div>
          ...
          <div className="mb-10">
            <p className="text-[10px] text-zinc-400 uppercase tracking-[0.2em] font-semibold mb-4">Tier 2 · AI sourcing tools</p>
            <p className="text-[14px] text-zinc-500 leading-[1.8] mb-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={comp.name} className="p-5 rounded-xl border border-zinc-200 bg-white">
                  <h4 className="text-[13px] font-bold text-zinc-900 mb-1">{comp.name}</h4>
                  <p className="text-[11px] font-medium text-amber-600 mb-2">{comp.funding}</p>
                  <p className="text-[12px] text-zinc-500 leading-[1.65] mb-3">{comp.desc}</p>
                  <p className="text-[11px] text-zinc-400 leading-[1.6] italic border-t border-zinc-100 pt-2">{comp.gap}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* ── TIER 3: CVin.Bio ── */}
          <div className="p-6 sm:p-8 rounded-2xl border border-zinc-900 bg-zinc-900 mb-6">
            <p className="text-[10px] text-zinc-400 uppercase tracking-[0.2em] font-semibold mb-3">Our category · Talent infrastructure</p>
            <h4 className="text-lg font-serif font-bold text-white mb-3">CVin.Bio</h4>
            <p className="text-[14px] text-zinc-300 leading-[1.8] mb-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={i} className="flex items-start gap-2 text-[11px] text-zinc-300 leading-[1.6]">
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Three revenue streams aligned with the hiring funnel</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div className="space-y-8">
              {[
                ...
                <div key={item.n} className="flex gap-5">
                  <span className="text-4xl font-serif font-bold text-zinc-200 shrink-0 leading-none pt-0.5">{item.n}</span>
                  <div>
                    <h3 className="text-[15px] font-bold text-zinc-900 mb-2">{item.title}</h3>
                    <p className="text-[14px] text-zinc-500 leading-[1.8]">{item.body}</p>
                  </div>
                </div>
              ))}
            </div>
            <div>
              <div className="bg-white border border-zinc-200 rounded-2xl p-8 mb-4">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-6">Revenue per unit economics</p>
                <HBar data={[
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Four reasons the timing is now</h2>
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-3">What we have shipped</h2>
          <p className="text-[14px] text-zinc-400 mb-8">Formally launched April 1, 2026. Entire platform self-built end to end.</p>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-zinc-200 rounded-2xl overflow-hidden mb-8">
            {[
              ...
              <div key={i} className="bg-[#fafafa] p-6 sm:p-8 text-center">
                <div className="text-2xl sm:text-3xl font-serif font-bold text-zinc-900 tracking-tight">{item.metric}</div>
                <div className="text-[11px] text-zinc-400 mt-2">{item.desc}</div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">The infrastructure layer for agent-mediated hiring</h2>
          <p className="text-[15px] text-zinc-500 leading-[1.85] mb-10 max-w-3xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-zinc-200 rounded-2xl overflow-hidden">
            {[
              ...
              <div key={i} className="bg-[#fafafa] p-6 sm:p-8">
                <div className="flex items-center gap-3 mb-5">
                  <div className="flex items-center justify-center w-7 h-7 rounded-full bg-zinc-900 text-white font-bold text-xs">{i + 1}</div>
                  <h3 className="text-sm font-bold text-zinc-900">{phase.phase}</h3>
                </div>
                <ul className="space-y-2.5">
                  {phase.items.map((item, j) => (
                    <li key={j} className="text-[13px] text-zinc-500 leading-[1.7] flex items-start gap-2">
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Built by someone who has done this before</h2>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white border border-zinc-200 rounded-2xl p-8 text-center">
                ...
                <div className="flex items-center justify-center gap-3 mb-6">
                  <a href="https://www.linkedin.com/in/vedangvatsa" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[12px] font-medium text-zinc-600 hover:text-zinc-900 transition-colors">
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                    LinkedIn
                  </a>
                  <span className="text-zinc-300">·</span>
                  <a href="https://veda.ng/profile" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[12px] font-medium text-zinc-600 hover:text-zinc-900 transition-colors">
                    Profile
                  </a>
                </div>
        ...
        <section className="mb-28">
          ...
          <h2 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight text-zinc-900 mb-8">Pre-seed round to reach first revenue</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              <p className="text-[15px] text-zinc-500 leading-[1.85] mb-5">
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={item.n} className="flex gap-5">
                    <span className="text-4xl font-serif font-bold text-zinc-200 shrink-0 leading-none pt-0.5">{item.n}</span>
                    <div>
                      <h3 className="text-[15px] font-bold text-zinc-900 mb-1">{item.title}</h3>
                      <p className="text-[13px] text-zinc-500 leading-[1.8]">{item.body}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div className="bg-white border border-zinc-200 rounded-2xl p-8 mb-4">
                <p className="text-xs font-medium text-zinc-400 uppercase tracking-widest mb-6">Planned milestones</p>
                <div className="space-y-5">
                  {[
                    ...
                    <div key={i} className="flex gap-4">
                      <span className="text-sm font-bold text-zinc-900 shrink-0 w-20">{item.q}</span>
                      <span className="text-[13px] text-zinc-500 leading-[1.7]">{item.items}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="p-10 bg-zinc-900 rounded-2xl text-center">
          <p className="text-sm text-zinc-400 mb-2">Interested in what we are building?</p>
          <p className="text-lg font-serif font-semibold text-white mb-5">Let&apos;s talk.</p>
          <Link
            href="/contact"
