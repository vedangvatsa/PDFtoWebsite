...
export default function ProfilePageClient({ data, slug }: Props) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: authData }) => {
      ...
      posthog.capture('celebration_shown', { slug });
        ...
        <DialogContent className="max-w-md text-center max-h-[90vh] overflow-y-auto p-0 overflow-hidden">
          ...
          <div className="flex flex-col items-center gap-4 px-6 py-5">
            ...
            <div className="w-full bg-secondary rounded-lg px-3 py-2.5 flex items-center justify-between gap-2">
              ...
              <button
                onClick={() => { navigator.clipboard.writeText(`https://cvin.bio/${slug}`); posthog.capture('share_link_copied', { slug, source: 'celebration' }); toast({ title: 'Copied!' }); }}
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
              <button
                onClick={() => {
                  ...
                  posthog.capture('share_linkedin', { slug });
              ...
              <button
                onClick={() => { posthog.capture('share_x', { slug }); window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(copiedX)}`, '_blank'); }}
              ...
              <button
                onClick={() => { posthog.capture('share_facebook', { slug }); window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://cvin.bio/${slug}`)}`, '_blank'); }}
              ...
              <button
                onClick={() => { posthog.capture('share_whatsapp', { slug }); window.open(`https://wa.me/?text=${encodeURIComponent(copiedWhatsApp)}`, '_blank'); }}
