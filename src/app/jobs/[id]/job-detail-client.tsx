...
export default function JobDetailClient({
  ...
  const CompanyApplyButton = ({ className = '' }: { className?: string }) => (
    <a
      ...
      rel="noopener noreferrer"
      onClick={trackClick}
      className={`inline-flex items-center justify-center gap-2 w-full rounded-xl bg-black text-white font-semibold px-4 py-3.5 text-sm sm:text-[15px] hover:scale-[1.01] active:scale-[0.99] transition-all shadow-md touch-manipulation ${className}`}
    >
      <span>Apply now</span>
  ...
  /** Primary CTA when the viewer already has a CV on file. */
  const ApplyPrimaryCard = () => (
    <div className="rounded-2xl border-2 border-zinc-900 bg-white p-5 sm:p-6 text-center">
      <Briefcase className="h-7 w-7 text-zinc-900 mx-auto mb-2.5" />
      <h2 className="text-base sm:text-lg font-bold text-zinc-900 tracking-tight">Ready to apply</h2>
      <p className="text-sm text-zinc-500 mt-1.5 mb-4 max-w-sm mx-auto leading-relaxed">
        Your profile is ready. Continue below to submit your application.
      </p>
      <CompanyApplyButton />
      <button
        type="button"
        onClick={() => router.push('/editor')}
        className="mt-3 text-sm font-medium text-zinc-500 hover:text-zinc-800 transition-colors"
      >
        Edit your CV first
      </button>
    </div>
  ...
  /** Primary conversion for guests or users without a parsed profile yet. */
  const CvPrimaryCard = () => (
    <div className="rounded-2xl border-2 border-zinc-900 bg-white p-5 sm:p-6 text-center">
